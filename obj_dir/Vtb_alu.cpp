...
std::unique_ptr<VerilatedTraceConfig> Vtb_alu::traceConfig() const {
...
void Vtb_alu___024root__trace_decl_types(VerilatedVcd* tracep);

void Vtb_alu___024root__trace_init_top(Vtb_alu___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vtb_alu___024root__trace_decl_types(tracep);
    Vtb_alu___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}
...
VL_ATTR_COLD void Vtb_alu::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vtb_alu::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    (void)levels; (void)options; // Prevent unused variable warning
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vtb_alu___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
